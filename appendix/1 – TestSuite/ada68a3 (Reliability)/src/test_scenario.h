...
private:
    std::unique_ptr<cti_client> m_client_ptr;
};

class test_scenario_server : public test_scenario
{
public:
    test_scenario_server(test_description description, std::shared_ptr<cti_server> server_ptr);
    bool start() override;
    bool stop() override;
...
private:
    std::shared_ptr<cti_server> m_server_ptr;
};
