 ...
 */
cti_client::cti_client(test_description description, bool udp) {
    ...
    case test_description::metadata::IPERF: {
        ...
        if(udp)
            m_cti_command += "-u ";
        m_cti_command += "-c " + std::string(m_description.connection.iperf.server_ip) + " ";
        m_cti_command += "-t " + std::to_string(m_description.duration) + " ";
        m_cti_command += "-b " + std::string(m_description.connection.iperf.bandwidth_limit) + " ";
        m_cti_command += "-l " + std::to_string(m_description.connection.iperf.datagram.size) + " ";
        break;
    }
...
void cti_client::start() {
    ...
    switch (m_description.metadata.method) {
    case test_description::metadata::IPERF:
        m_thread_ptr = std::unique_ptr<std::thread>(new std::thread(iperf_thread, m_cti_command, std::ref(m_cti_output)));
        break;
    case test_description::metadata::CUSTOM:
...
test_results cti_client::get_results() {
    ...
    switch (m_description.metadata.method) {
    case test_description::metadata::IPERF:
        if(m_cti_output.find("Client connecting") != std::string::npos) {
            std::size_t pos_start = m_cti_output.find('\n') + 1;
            std::size_t pos_end = m_cti_output.find('-', pos_start) - 1;
            
            m_results.iperf.startup_message = m_cti_output.substr(pos_start, pos_end-pos_start);
        }
        else {
        ...
        if(m_cti_output.find("Connection refused") != std::string::npos) {
            m_results.status = test_results::status::STATUS_FAIL;
            return m_results;
        }
        
        if(m_cti_output.find("Server Report") != std::string::npos) {
            m_results.status = test_results::status::STATUS_SUCCESS;
            
            m_cti_output.pop_back();
            std::string summary_line = m_cti_output.substr(m_cti_output.rfind('\n')+1);
...
//      (_-</ -_) '_\ V / -_) '_|
//  cti_/__/\___|_|  \_/\___|_|
//

cti_server::cti_server(enum test_description::metadata::method method, int datagramsize, bool udp)
{
    ...
    switch(m_description.metadata.method) {
    case test_description::metadata::IPERF: {
        m_cti_command  = "iperf -s -w 50M";
        if(udp)
            m_cti_command += "-u ";
        m_cti_command += "-l " + std::to_string(datagramsize) + " ";
        break;
    }
    case test_description::metadata::CUSTOM: {
        break;
...
void cti_server::load_test(test_description description) {
    m_description = description;
...
test_results cti_server::get_results() {
    if(m_thread_ptr == nullptr) {
    ...
    switch (m_description.metadata.method) {
    case test_description::metadata::IPERF: {
        if(m_cti_output.find("connected with") != std::string::npos) {
            m_results.status = test_results::status::STATUS_SUCCESS;
            
            m_results.iperf.startup_message = m_cti_output;
