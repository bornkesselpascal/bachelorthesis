...
bool test_scenario_client::start() {
    try {
        m_client_ptr = std::unique_ptr<cti_client>(new cti_client(m_description));
        m_client_ptr->start();
...
test_scenario_server::test_scenario_server(test_description description, std::shared_ptr<cti_server> server_ptr)
    : test_scenario(description)
